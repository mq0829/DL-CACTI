function  [im, PSNR_save]    =   TV_ADMM_CACTI_FFD( y, para, M_func,Mt_func)
     ...
     for iter = 1:para.maxiter(isig-1)
       ...
       %  for ii = 1 : 1
               fpb = f+b;
                ...
                for ncc = 1:nc
                f(:,:,ncc)         =  FFD_Net_Denoiserfast(squeeze(theta(:,:,ncc)-b(:,:,ncc)), sig_use,para.net); %(f,  para.TVweight,5);  TV_GAP_CACTI_FFDnet
                end
                %f = f./255;
               b = b -theta+f;  
      end
