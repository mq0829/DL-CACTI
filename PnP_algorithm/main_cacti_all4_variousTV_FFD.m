...
Phi_sum(Phi_sum==0)=1;
 ...
 para.sigma   = [200 100 50 12  6]; % noise deviation (to be estimated and adapted)
  para.vrange   = 1; % range of the signal
  % para.maxiter = [ 60 60 60 60  60];
  para.maxiter = [20 20 20 20 20]; % first trial
 
  TV_m_all = {'ATV_ClipA', 'ATV_ClipB','ATV_cham','ATV_FGP','ITV2D_cham','ITV2D_FGP','ITV3D_cham','ITV3D_FGP'};
  
  addpath(genpath('.\LR_FFDNet_CS_Test_BSD_68'));
for k=1:CodeFrame        
...
%          
            ...
            % theta_gap{nm}(:,:,(k-1)*ColT+(1:ColT)) = TV_GAP_CACTI_all( y_use, para,A,At); 
              theta_gap{nm}(:,:,(k-1)*ColT+(1:ColT)) = TV_GAP_CACTI_FFDnet( y_use, para,A,At); 
...
save([fname '_all4_variousTV_20190930.mat']); 
