function [X_den,iter,fun_all]=fgp_denoise_bound_3d(Xobs,lambda,l,u,pars)
...
while((i<MAXITER)&(count<5))
    ...
    P{3}=R{3}+1/(8*lambda*mu)*Q{3};
    
    %%%%%%%%%%
    % Peforming the projection step
    switch tv
        case 'iso'
            A=[P{1};zeros(1,n,B)].^2+[P{2},zeros(m,1,B)].^2+[P{3},zeros(m,n,1)].^2;
            A=sqrt(max(A,1));
            P{1}=P{1}./A(1:m-1,:,:);
            P{2}=P{2}./A(:,1:n-1,:);
            P{3}=P{3}./A(:,:,1:B-1);
