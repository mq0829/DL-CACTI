...
for kf = 1 : nframe
    ...
    for ibayer = 1:nbayer % Bayer element-wise
        ...
        switch lower(projmeth)
            ...
            case 'admm_res' % ADMM-Denoise with residual in ADMM
                if isfield(para,'wnnm_int') && para.wnnm_int % GAP-WNNM integrated
                    if isempty(orig) || (isfield(para,'flag_iqa') && ~para.flag_iqa) % ImQualAss disabled
                        v = admmwnnm_int_res(yb,para);
                    else
                        [v,psnrall(kf,:)] = admmwnnm_int_res(yb,para);
                    end
                else
                    if isempty(orig) || (isfield(para,'flag_iqa') && ~para.flag_iqa) % ImQualAss disabled
                        v = admmdenoise_res(yb,para);
                    else
                        [v,psnrall(kf,:)] = admmdenoise_res(yb,para);
                    end
                end
            otherwise
...
if ~isempty(orig)
    for kv = 1:nmask*nframe
        psnr_(kv) = psnr(double(v_(:,:,kv)),orig(:,:,kv)/MAXB,255/MAXB);
        ssim_(kv) = ssim(double(v_(:,:,kv)),orig(:,:,kv)/MAXB);
    end
end
