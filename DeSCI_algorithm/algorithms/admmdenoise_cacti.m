...
for kf = 1:nframe
    ...
    switch lower(projmeth)
        ...
        case 'admm_res' % ADMM-Denoise with residual in ADMM
            if isfield(para,'wnnm_int') && para.wnnm_int % GAP-WNNM integrated
                if isfield(para,'flag_iqa') && ~para.flag_iqa % ImQualAss disabled
                    v = admmwnnm_int_res(y,para);
                else
                    [v,psnrall(kf,:)] = admmwnnm_int_res(y,para);
                end
            elseif isfield(para,'wnnm_int_fwise') && para.wnnm_int_fwise % GAP-WNNM integrated (with frame-wise denoising)
                if isfield(para,'flag_iqa') && ~para.flag_iqa % ImQualAss disabled
                    v = admmwnnm_int_res_fwise(y,para);
                else
                    [v,psnrall(kf,:)] = admmwnnm_int_res_fwise(y,para);
                end
            else
                if isfield(para,'flag_iqa') && ~para.flag_iqa % ImQualAss disabled
                    v = admmdenoise_res(y,para);
                else
                    [v,psnrall(kf,:)] = admmdenoise_res(y,para);
                end
            end
        otherwise
