function [v,psnrall] = admmdenoise_res( y , opt )
%ADMMDENOISE_RES Alternating direction method of multipliers (ADMMM)-based 
%denoising framework for compressive sensing reconstruction.
%   v=ADMMDENOISE_RES(y,opt) returns the reconstruction result v of the
...
%       update Jan 12, 2018.
% 
%   See also TEST_ADMMDENOISE, GAPDENOISE, ADMMDENOISE.
if nargin<2
    opt = [];
end
...
for isig = 1:length(sigma)
    ...
    %             y1 = y1+(y-yb);
    %             v = (theta+b)+lambda*(At((y1-yb)./(Phisum+gamma))); % lambda=1 for ADMM
    ...
    %         end
        ...
        switch lower(denoiser)
            ...
            case 'bm4d' % BM4D denoising
                theta = bm4d(v-b,'Gauss',0,'lc',1,0);
            case 'wnnm_c' % WNNM video denoising (C-style strucuture version)
                theta = wnnmvdenoiser(v-b,[],[],opt); % opt.sigma
        ...
        if flag_iqa && isfield(opt,'orig') && (~isempty(opt.orig))
            ...
            if (mod(k,5)==0) 
                fprintf('  GAP-%s iteration % 4d, sigma %.1f, PSNR %2.2f dB.\n',...
                    upper(opt.denoiser),k,nsigma,psnrall(k));
            end
        end
        k = k+1;
