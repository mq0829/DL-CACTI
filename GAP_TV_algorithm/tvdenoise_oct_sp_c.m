function u = tvdenoise_oct_sp_c(f,lambda,iters)
...
%  (stopping rule: iters)
%  Optimized to reduce run time by Mike Scheutzow 2018
%   

assert(isa(f, 'single'), 'f must be type single');
...
% divp must not be a shared buffer, so allocate it
...
f_lambda = -1*lambda*f;
tvden(divp, iters, f_lambda, dt, ncpu);
