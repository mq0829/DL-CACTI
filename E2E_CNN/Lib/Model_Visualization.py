...
def generate_folder_id(model_config):
    ...
    drop_rate_forward = float(model_config.get('drop_rate_forward'))
    learning_rate_init = float(model_config.get('learning_rate'))
    loss_func = model_config.get('loss_func')

    # Generate the folder name
    folder_id = '%s-T%s-D%.2fL%.3f-%s/' % (
        model_name, time.strftime('%m%d%H%M%S'), drop_rate_forward, learning_rate_init, loss_func)
